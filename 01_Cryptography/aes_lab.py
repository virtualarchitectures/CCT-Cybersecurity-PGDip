...
def text_to_hex_matrix(plaintext):
    ...
    for i, char in enumerate(plaintext):
        ...
        row = i % 4
        col = i // 4
        # Use ord(char) to get ASCII value
...
def sub_bytes(state_matrix):
    ...
    # For each byte in the matrix, use its value to look up the S-box

    # High 4 bits = row, low 4 bits = column
    # Replace the byte with the S-box value

    # **Hint:** Use `byte >> 4` for row and `byte & 0x0F` for column.
...
if __name__ == "__main__":
    ...
    # Test text_to_hex_matrix
    test_text = "ABCDEFGHIJKLMNOP"
    matrix = text_to_hex_matrix(test_text)
    print_matrix(matrix, f"Text '{test_text}' as Matrix")
